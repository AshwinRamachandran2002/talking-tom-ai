...
bool bark_forward_text_encoder_setup_compute(struct bark_context* bctx, int n_threads) {
    const int64_t t_main_start_us = ggml_time_us();

    auto& model = bctx->text_model.semantic_model;
    auto& allocr = bctx->allocr->semantic_allocr;
    auto& hparams = model.hparams;
    auto& verbosity = bctx->params.verbosity;
    ...
    // allocate the compute buffer
    {
        // alignment required by the backend
        size_t align = ggml_backend_get_alignment(model.backend);
        bctx->allocr->semantic_allocr = ggml_allocr_new_measure(align);
        ...
        // recreate the allocator with the required memory
        ggml_allocr_free(bctx->allocr->semantic_allocr);
        bctx->buf_compute->semantic_buf_compute = ggml_backend_alloc_buffer(model.backend, mem_size);
        bctx->allocr->semantic_allocr = ggml_allocr_new_from_buffer(bctx->buf_compute->semantic_buf_compute);
...
bool bark_free_compute(struct bark_context* bctx) {

    ggml_backend_buffer_free(bctx->buf_compute->semantic_buf_compute);
    ggml_allocr_free(bctx->allocr->semantic_allocr);

    ggm_backend_buffer_free(bctx->buf_compute->fine_buf_compute);
    ggml_allocr_free(bctx->allocr->fine_allocr);

    ggml_backend_buffer_free(bctx->buf_compute->coarse_buf_compute);
    ggml_allocr_free(bctx->allocr->coarse_allocr);
...
static bool bark_forward_eval_stream(struct bark_context* bctx, int n_threads) {
    // divide threads among the three encoders
    // For now, 20:60:20 split
    int n_threads_text = n_threads / 5;
    int n_threads_fine = n_threads / 5;
    int n_threads_coarse = n_threads - n_threads_text - n_threads_fine;

    // Set compute buffer for all first
    if (!bark_forward_text_encoder_setup_compute(bctx, n_threads)) {
        fprintf(stderr, "%s: failed to set up compute for forward text encoder\n", __func__);
        return false;
    }

    if (!bark_forward_coarse_encoder_setup_compute(bctx, n_threads)) {
        fprintf(stderr, "%s: failed to set up compute for forward coarse encoder\n", __func__);
        return false;
    }

    if (!bark_forward_fine_encoder_setup_compute(bctx, n_threads)) {
        fprintf(stderr, "%s: failed to set up compute for forward fine encoder\n", __func__);
        return false;
    }
